...
int Convert(const char *input_filename, const char *output_filename, const char *overlay_name)
{
	...
	char id_string[512]; // will hold the Placemark ID
	char full_string[4096]; // will hold nearly all of the <Placemark> data until deciding to retain ID or drop it

	int type_string_length = 0; // length of type_string[]
	int id_string_length = 0; // length of id_string[]
	int full_string_length = 0; // length of full_string[]

	int folder_flag = 0; // points code to look for the next <name> tag after the <Folder> tag

	int entity_flag = 0; // points code to look for the next <SimpleData name="ViperTag">
	...
	while (bytes > 0) // read to end of file
	{
		...
		if (bytes > 0)
		{
			...
			if (entity_flag == 0) fprintf(output, "%c", buffer); // output to file if not in <Placemark>
			else if (entity_flag == 1)
			{
				full_string[full_string_length] = buffer;
			...
			if (strcmp(test_string, "<SimpleData name=\"ViperType\">") == 0) // compare and see if this is an Entity
			{
				...
				{
					fprintf(output, "%s%s%c", full_string, "MapPointIcon", buffer); // convert them into MapPointIcon's
				}
				else
				{
					fprintf(output, "%s%s%s%c", id_string, full_string, type_string, buffer); // otherwise, ignore them
				}
			...
			if (strcmp(test_string, "<Placemark") == 0) // compare and see if this is a Placemark
			{
				...
				while (buffer != '>')
				{
					...
					if (buffer != '<')
					{
						id_string[id_string_length] = buffer; // store the type of entity in id_string[]
				...
				// add buffer to full_string[]
				full_string[full_string_length] = buffer; // always a > symbol
				
				full_string_length++;
			}

			// Test for Folder... (should NEVER be while entity_flag = 1;)
